...
P( 2 * closestNode( nodes, [ lx      ly ] ) ) = P0; % [N]
P( 2 * closestNode( nodes, [ lx-dx   ly ] ) ) = P0; % [N]
P( 2 * closestNode( nodes, [ lx-2*dx ly ] ) ) = P0; % [N]
P( 2 * closestNode( nodes, [ lx-3*dx ly ] ) ) = P0; % [N]
P( 2 * closestNode( nodes, [ lx-4*dx ly ] ) ) = P0; % [N]
P( 2 * closestNode( nodes, [ lx-5*dx ly ] ) ) = P0; % [N]
...
[ u, GPdatas, corr ] = incrementalPlaneStressPlasticAnalysis( elemClassL4, nodes, elems, 0.7 * c * P, material, profile, supports, x );
...
%subplot(1,2,1), colormap('default'), imagesc(flipud(reshape(ais, nx, ny)')), 
%view(2), axis image, axis off, colorbar
%title(['Iter: ', num2str(iter),', Stress distribution']);

%contour( reshape(ais,nx,ny)', 'ShowText', 'on'),
%subplot(1,2,2), colormap(gray), imagesc( -flipud(reshape(x,nx,ny)')), axis image, axis tight, axis off
...
title(['Iter: ', num2str(iter),', nr: ', num2str(nRemoved),', Volume: ' num2str(V), ' m^3']);
saveas(gcf,['plasticCantilever2_', num2str(iter),'.png'] );
