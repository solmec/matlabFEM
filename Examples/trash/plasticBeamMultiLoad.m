...
for k=1:nlcases
    [ ~, ~, c(k) ] = planeStressPlasticAnalysisCapacity( elemClassL4, nodes, elems, mP(:,k), material, profile, supports, x );
end
...
while true
    ...
    GPdatas = [];    
    for k=1:nlcases
        [ ~, GPdata, corr ] = incrementalPlaneStressPlasticAnalysis( elemClassL4, nodes, elems, 0.5 * c(k) * mP(:,k), material, profile, supports, x );
        GPdatas = [GPdatas; GPdata];
        if ( corr == false )
            x            = xPrev;
