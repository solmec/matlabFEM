...
%P( 2 * closestNode( nodes, [ lx/4+dx 0 ] ) -1 ) = 0.0; % [N]
P( 2 * closestNode( nodes, [ lx/2    0 ] ) -1 ) = 0.0; % [N]
%P( 2 * closestNode( nodes, [ lx/4-dx 0 ] ) -1 ) = 0.0; % [N]

%P( 2 * closestNode( nodes, [ lx/4+dx 0 ] ) ) = P0; % [N]
P( 2 * closestNode( nodes, [ lx/2    0 ] ) ) = P0; % [N]
%P( 2 * closestNode( nodes, [ lx/4-dx 0 ] ) ) = P0; % [N]

%P( 2 * closestNode( nodes, [ 3*lx/4+dx 0 ] ) -1 ) = 0.0; % [N]
P( 2 * closestNode( nodes, [ lx    0 ] ) -1 ) = 0.0; % [N]
%P( 2 * closestNode( nodes, [ 3*lx/4-dx 0 ] ) -1 ) = 0.0; % [N]

%P( 2 * closestNode( nodes, [ 3*lx/4+dx 0 ] ) ) = P0; % [N]
...
%P( 2 * closestNode( nodes, [ lx/4+dx 0 ] ) -1 ) = -Px; % [N]
P( 2 * closestNode( nodes, [ lx/2    0 ] ) -1 ) = -Px; % [N]
%P( 2 * closestNode( nodes, [ lx/4-dx 0 ] ) -1 ) = -Px; % [N]

%P( 2 * closestNode( nodes, [ lx/4+dx 0 ] ) ) = Py; % [N]
P( 2 * closestNode( nodes, [ lx/2    0 ] ) ) = Py; % [N]
%P( 2 * closestNode( nodes, [ lx/4-dx 0 ] ) ) = Py; % [N]

%P( 2 * closestNode( nodes, [ 3*lx/4+dx 0 ] ) -1 ) = -Px; % [N]
P( 2 * closestNode( nodes, [ lx    0 ] ) -1 ) = -Px; % [N]
%P( 2 * closestNode( nodes, [ 3*lx/4-dx 0 ] ) -1 ) = -Px; % [N]

%P( 2 * closestNode( nodes, [ 3*lx/4+dx 0 ] ) ) = Py; % [N]
