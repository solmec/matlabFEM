...
nelem = size(elems,1);
 ...
 supports  = [ nearestTo( nodes, [ 0, 0 ], [ 1 1 ] ) ...
                                              ...
                                              nearestTo( nodes, [ lx,   0 ], [ 1 1 ] ) ...
                                              nearestTo( nodes, [ lx-dx, 0 ], [ 1 1 ] ) ...
                                              nearestTo( nodes, [ lx-2*dx, 0 ], [ 1 1 ] )...
                                              nearestTo( nodes, [ lx-3*dx, 0 ], [ 1 1 ] )...
                                              nearestTo( nodes, [ lx-4*dx, 0 ], [ 1 1 ] )...
                                              nearestTo( nodes, [ lx-5*dx, 0 ], [ 1 1 ] )...
                                              nearestTo( nodes, [ lx-6*dx, 0 ], [ 1 1 ] )...
                                              nearestTo( nodes, [ lx-7*dx, 0 ], [ 1 1 ] )...
                                              nearestTo( nodes, [ lx-8*dx, 0 ], [ 1 1 ] )...
                                              nearestTo( nodes, [ lx-9*dx, 0 ], [ 1 1 ] )];
...
P( 2 * closestNode( nodes, [ lx / 2 + 2*dx 0 ] )- 1 ) = Px; % [N]
P( 2 * closestNode( nodes, [ lx / 2 - dx   0 ] )- 1 ) = Px; % [N]
P( 2 * closestNode( nodes, [ lx / 2 - 2*dx 0 ] )- 1 ) = Px; % [N]

P( 2 * closestNode( nodes, [ lx / 2    0 ] ) ) = Py; % [N]
P( 2 * closestNode( nodes, [ lx / 2 + dx   0 ] ) ) = Py; % [N]
P( 2 * closestNode( nodes, [ lx / 2 + 2*dx 0 ] ) ) = Py; % [N]
P( 2 * closestNode( nodes, [ lx / 2 - dx   0 ] ) ) = Py; % [N]
P( 2 * closestNode( nodes, [ lx / 2 - 2*dx 0 ] ) ) = Py; % [N]
...
P( 2 * closestNode( nodes, [ lx / 2    0 ] ) - 1) = 0; % [N]
P( 2 * closestNode( nodes, [ lx / 2 + dx   0 ] )- 1 ) = 0; % [N]
P( 2 * closestNode( nodes, [ lx / 2 + 2*dx 0 ] )- 1 ) = 0; % [N]
P( 2 * closestNode( nodes, [ lx / 2 - dx   0 ] )- 1 ) = 0; % [N]
P( 2 * closestNode( nodes, [ lx / 2 - 2*dx 0 ] )- 1 ) = 0; % [N]
...
xplus  = plasticTopologyOptimization( 'plasticBeam11XPlus', nodes, elems, elemClassL4, mP(:,1), material, profile, supports, x, lx, ly, nx, ny );
xminus = plasticTopologyOptimization( 'plasticBeam11XMinus', nodes, elems, elemClassL4, mP(:,2), material, profile, supports, x, lx, ly, nx, ny );
xvert  = plasticTopologyOptimization( 'plasticBeam11Vert', nodes, elems, elemClassL4, mP(:,3), material, profile, supports, x, lx, ly, nx, ny );

xenv = max( max( xplus, xminus) , xvert );
...
saveas( gcf,'plasticBeam11_envelope.png' );
savefig('plasticBeam11_envelope');
