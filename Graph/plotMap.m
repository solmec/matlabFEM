...
%   Detailed explanation goes here
    ...
    elseif  strcmp(type,'stress')
         colormap(jet), imagesc( flipud( reshape(x,nx,ny)' ) ), axis image, axis tight, axis off, colorbar
         title(makeTitle(iter, nRemoved, V, V0, fpen, rmin));
    elseif  strcmp(type,'stress_cut')
            ...
            map = [ 1 1 1; jet ];
            colormap(map), imagesc( flipud(-reshape( plotX, nx, ny )')), axis image, axis tight, axis off, colorbar
            title(makeTitle( iter, nRemoved, V, V0, fpen, rmin) );
    end
