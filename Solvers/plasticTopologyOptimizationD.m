...
while true
    ...
    [ u, GPdatas, corr ] = incrementalPlaneStressPlasticAnalysisD ( elemClass, nodes, elems, P, material, profile, supports, x );
...
x( erased_elems ) = max( 0.01*profile.h, x( erased_elems ) .* ais( erased_elems ).^fpen); % 1.5 is perfect!!!!
V = lx*ly*profile.h*sum(x)/nx/ny;
